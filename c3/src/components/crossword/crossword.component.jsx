...
class Crossword extends Component {
    ...
    handleClick = async (key) => {
        ...
        for (let i = 0; i < questionArray.length; i++) {
            ...
            var today;
            await axios.get('/liveevents/gettime')
                .then(res => {
                    today = res.data.time;
                });
            let date = today.date, hour = today.hour, min = today.min, sec = today.sec;
    ...
    componentDidMount() {
        let questionColor=[];
        for (let i = 0; i < 30 * 31; i++) questionColor.push("red");
        questionColor[10] = "green";
        this.setState({
            questionColor: questionColor
        })
        axios.get('/auth/c3status')
            .then(res => {
                if (!res.data.valid) {
                    Swal.fire({
                        icon: 'error',
                        title: 'No Access!',
                        text: 'You are not logged in or you have not registered for this event!'
                    })
                        .then(data => {
